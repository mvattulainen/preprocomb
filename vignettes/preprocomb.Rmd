...
testdata <- droplevels(testdata[1:70,]) # add class imbalance
```
...
the preprocessing pipeline consists 3200 combinations and their evaluation. The
evaluation took 2 hours on Intel Celeron 1.4 GHz and two cores. 
```{r, messages=FALSE, warnings=FALSE, eval=FALSE}
examplegrid <- setgrid(phases=preprodefault, data=testdata)
exampleresult <- preprocomb(grid=examplegrid, nholdout=4, cores=2)
